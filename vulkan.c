...
vk_buffer_allocate_common(VulkanBuffer *vb, VulkanBufferAllocateInfo *ai)
{
	 ...
	 */
	vb->memory_kind = ai->host_read_write ? VulkanMemoryKind_BAR : VulkanMemoryKind_Device;
...
vk_buffer_allocate(GPUBuffer *b, GPUBufferAllocateInfo *info)
{
	...
	VulkanBufferAllocateInfo vulkan_buffer_allocate_info = {
		...
		.export          = info->export,
		.size            = (u64)info->size,
		.host_read_write = info->host_read_write,
...
vk_image_allocate(GPUImage *image, u32 width, u32 height, u32 mips, u32 samples,
                  VulkanImageUsage usage, VulkanImageUsageFlags flags, OSHandle *export)
{
	...
	if (flags & VulkanImageUsageFlag_Sampling)            usage_flags |= VK_IMAGE_USAGE_SAMPLED_BIT;
	if (flags & VulkanImageUsageFlag_TransferSource)      usage_flags |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
	if (flags & VulkanImageUsageFlag_TransferDestination) usage_flags |= VK_IMAGE_USAGE_TRANSFER_DST_BIT;
