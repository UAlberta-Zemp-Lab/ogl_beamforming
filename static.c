...
static beamformer_complete_compute_fn     *beamformer_complete_compute;
static beamform_work_queue_push_fn        *beamform_work_queue_push;
static beamform_work_queue_push_commit_fn *beamform_work_queue_push_commit;
...
static FILE_WATCH_CALLBACK_FN(debug_reload)
{
	...
	beamform_work_queue_push = os_lookup_dynamic_symbol(debug_lib,
	                                                      "beamform_work_queue_push", &err);
	beamform_work_queue_push_commit = os_lookup_dynamic_symbol(debug_lib,
	...
	while (!platform->compute_worker.asleep);

	beamformer_complete_compute = os_lookup_dynamic_symbol(debug_lib,
	                                                       "beamformer_complete_compute", &err);
