...
setup_beamformer(BeamformerCtx *ctx, Arena *memory)
{
	...
	#define X(e, sn, f, nh, pretty_name) do if (s8(f).len > 0) {                       \
		...
		csr->shader       = sn;                                                    \
		csr->needs_header = nh;                                                    \
		reload_compute_shader(s8("shaders/" f ".glsl"), (iptr)csr, *memory);       \
		os_add_file_watch(&ctx->platform, memory, s8("shaders/" f ".glsl"),        \
		                  reload_compute_shader, (iptr)csr);                       \
	} while (0);
	COMPUTE_SHADERS
	#undef X

	reload_render_shader(s8("shaders/render.glsl"), (iptr)&ctx->fsctx, *memory);
	os_add_file_watch(&ctx->platform, memory, s8("shaders/render.glsl"), reload_render_shader,
	                  (iptr)&ctx->fsctx);
