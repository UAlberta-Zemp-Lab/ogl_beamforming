...
complete_queue(BeamformerCtx *ctx, BeamformWorkQueue *q, Arena *arena)
{
	...
	while (work) {
		...
		case BeamformerWorkKind_Compute:{
			...
			b32 has_das = 0, has_sum = 0;
			for (u32 i = 0; i < cp->pipeline.shader_count; i++) {
				has_das |= cp->pipeline.shaders[i] == BeamformerShaderKind_DAS;
			...
			if (has_das) {
				BeamformerShaderBufferClearPushConstants pc = {
					.data       = cs->backlog.buffer->gpu_pointer + frame->buffer_offset,
					.clear_word = 0,
				...
				vk_command_push_constants(cmd, 0, sizeof(pc), &pc);
				vk_command_dispatch_compute(cmd, (uv3){
					.x = (u32)ceil_f32((f32)pc.words / vk_gpu_info()->subgroup_size),
					.y = 1,
					.z = 1,
				});
			}
