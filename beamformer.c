...
function m4
z_aligned_volume_transform(v3 extent, v3 translation, f32 rotation_turns)
{
	m4 S, R, T;
	extent.y = MAX(0.005, extent.y);
	S.c[0] = (v4){{extent.x, 0,        0,        0}};
	S.c[1] = (v4){{0,        extent.z, 0,        0}};
	S.c[2] = (v4){{0,        0,        extent.y, 0}};
	S.c[3] = (v4){{0,        0,        0,        1}};

	f32 sa = sin_f32(rotation_turns * 2 * PI);
	f32 ca = cos_f32(rotation_turns * 2 * PI);
	R.c[0] = (v4){{ ca, 0, sa, 0}};
	R.c[1] = (v4){{ 0,  1, 0,  0}};
	R.c[2] = (v4){{-sa, 0, ca, 0}};
	R.c[3] = (v4){{ 0,  0, 0,  1}};
