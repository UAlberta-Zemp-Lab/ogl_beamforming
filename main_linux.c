...
load_platform_libraries(BeamformerInput *input)
{
	...
	input->vulkan_library_handle = BeamformerInvalidHandle;
	#define X(name) \
		if (input->vulkan_library_handle.value[0] == BeamformerInvalidHandle.value[0])\
			input->vulkan_library_handle = load_library(name, 0, RTLD_NOW|RTLD_LOCAL);
	OS_VULKAN_SONAME_LIST
	#undef X

	if (!ValidHandle(input->vulkan_library_handle))
		os_fatal(s8("[os] fatal error: failed to find valid vulkan library\n"));
